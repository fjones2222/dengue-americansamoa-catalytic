...
svyglm(sero_binary ~ birth_group, design = as.dsn, family = quasibinomial()) %>%
      summary()
svyglm(sero_binary ~ ob_location, design = as.dsn, family = quasibinomial()) %>% summary()
...
ggsave(filename="output/figures/figure3.png", plot=f3)
...
f4a_p2<- f4a_p2_df%>%
      ...
      ggplot(aes(x=fct_rev(setting),y=cumhaz_to_prob(.value)))+
      geom_point()+
      geom_linerange(aes(ymin=cumhaz_to_prob(.lower),ymax=cumhaz_to_prob(.upper)))+
      theme_minimal()+
...
cohort_df <- long_dataset %>%
      filter(birthplace_clean %in% c("Australia","New Zealand","USA")) %>%
...
read_rds(file="output/model_fits/20250603_2241_former_combo1718.rds")%>% 
      spread_draws(lambda[year]) %>%
      ungroup()%>%
      mutate(year=2023-max(year)+year) %>%
...
modelobject<- read_rds(file="output/model_fits/20250603_2142_former.rds")
